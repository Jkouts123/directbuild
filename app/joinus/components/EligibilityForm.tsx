...
const SERVICE_STATE_OPTIONS: AustralianState[] = [
  "NSW",
  "VIC",
  "QLD",
  "WA",
...
export default function EligibilityForm({
    ...
    setForm((p) => {
      ...
      const service_region_ids = selected
        ? p.service_region_ids.filter(
            (id) => getServiceRegionById(id)?.state !== state,
          )
  ...
  async function submitLive() {
    ...
    const payload = {
      ...
      location_based_in: serviceArea,
      locations_serviced: serviceArea,
      phone: form.phone,
      email: form.email,
      verified_phone: true,
          ...
          >
            ...
            <Field>
              <span className={LABEL}>
                Verify your business ABN
              </span>
              <AbnSearch value={abn} onChange={handleAbnChange} />
              <p className={HINT}>
                We use this to confirm you operate under a registered
                Australian business.
              </p>
            </Field>
            ...
            <Field>
              <span className={LABEL}>Which state/s do you service?</span>
              <StateSelectGroup
            ...
            <Field>
              <span className={LABEL}>
                Where can you reliably take on private residential work?
              </span>
              <p className={HINT}>
                Select the areas you can actually service. We use this to match
                homeowner enquiries and check local market signals.
              </p>
              <p className={HINT}>Pick one or more service regions.</p>
              ...
              <p className={HINT}>
                ...
                {form.service_area || "Choose at least one region."}
              </p>
            </Field>
...
function StateSelectGroup({
  ...
  return (
    <div className="grid grid-cols-4 gap-2.5" aria-label="service states">
      {SERVICE_STATE_OPTIONS.map((state) => {
...
function ServiceRegionSelectGroup({
  ...
  if (selectedStates.length === 0) {
    return (
      <div className="mt-3 rounded-lg border border-white/10 bg-white/[0.025] px-4 py-4 text-sm text-white/50">
        Select a state first to see available service regions.
      </div>
    );
  }
